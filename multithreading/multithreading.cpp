...
public:
	...
	VulkanExample() : VulkanExampleBase(ENABLE_VALIDATION)
	{
        ...
        int nt = atoi(args[2]);
        index_mesh = atoi(args[3]);
        scale = atof(args[4]);
        std::cout << "mesh  = " << ar_models[index_mesh] << std::endl;
        ...
        std::cout << "number of objects = " << number_objects << std::endl;
		threadPool.setThreadCount(numThreads);

		numObjectsPerThread = number_objects / numThreads;
	}
	...
	void prepareMultiThreadedRenderer()
	{
		...
		for (uint32_t i = 0; i < numThreads; i++)
		{
			...
			for (uint32_t j = 0; j < numObjectsPerThread; j++)
			{
				...
				thread->objectData[j].pos = glm::vec3(rand() % 100, 0.0f, rand() % 100);	//POSICION
