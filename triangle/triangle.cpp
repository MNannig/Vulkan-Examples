...
public:
	...
	void prepareVertices(bool useStagingBuffers, int cant)
	{
		...
		int cant_tri = cant;
		int cant_puntos = cant_tri*2+2; //CANTIDAD DE PUNTOS = CANT TRIANGULOS*2 +2
		int cont = 0;
		...
		int j = 0;
		for (int e = 0; e < cant; ++e)			//CANTIDAD DE LINEAS DE TRIANGULOS
		{
			pos_x = -100.0;
			for (int i = 0; i < cant_puntos; ++i)
			{
				if (i%2==0)
				{
					if(i==0){
						pos_y = dist_y*e;
						vertexBuffer.push_back({{pos_x, pos_y, pos_z},{amar1, amar1, amar2}});
					}
					else
					{
						pos_y = dist_y*e;
			...
			for (int i = 0; i < cant_tri; ++i)
			{
				...
				cont = 0;
				j=j-1;
				//printf("triangulo i %i\n", i);
				
			}
			j=j+2;
		}
		printf("tamaño vertices %lu\n", sizeof(vertexBuffer));
		printf("tamaño indexBuffer %lu\n", sizeof(indexBuffer));
	...
	void prepare(int aux)
	{
		VulkanExampleBase::prepare();
		prepareSynchronizationPrimitives();
		prepareVertices(USE_STAGING, aux);
